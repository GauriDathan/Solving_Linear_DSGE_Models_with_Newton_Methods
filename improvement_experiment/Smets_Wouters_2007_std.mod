...
parameters curvw cgy curvp constelab constepinf constebeta cmaw cmap calfa 
    ...
    ctrend cg
    sea seb seg seqs sem sepinf sew;
//noisence
...
%load('Smets_Wouters_2007_mean.mat')
%xdone=xparam1(8:end);
...
%           %eval(['M_.params(' int2str(parameter_index) ')=' paramname ';']);
%        end
...
// sticky price - wage economy
	      ...
	      a = crhoa*a(-1)  + sea*ea;
	      b = crhob*b(-1) + seb*eb;
	      g = crhog*(g(-1)) + seg*eg + cgy*sea*ea;
	      qs = crhoqs*qs(-1) + seqs*eqs;
	      ms = crhoms*ms(-1) + sem*em;
	      spinf = crhopinf*spinf(-1) + epinfma - cmap*epinfma(-1);
	          epinfma=sepinf*epinf;
...
// estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_compute=0,mode_file=usmodel_shock_decomp_mode,first_obs=71,presample=4,lik_init=2,prefilter=0,mh_replic=250000,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2,bayesian_irf,irf=20,moments_varendo,filtered_vars,smoother);

//shock_decomposition y;

   stoch_simul(nograph, nocorr, nodecomposition, nofunctions, noprint,irf=0) ;

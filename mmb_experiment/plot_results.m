...
% mmb experiment with starting guess zero matrix
% Paper: table 5, figure 4, figure 5, figure 6, figures in appendix 6.5
...
model_variable_number=sum(squeeze(AMG_JS_Results(1,3:4,:)));

%calculate relative times
times=squeeze(AMG_JS_Results(2:end,1,:));
iterations=squeeze(AMG_JS_Results(2:end,7,:)); iterations(1,:)=1;
...
%Gather the results in a matrix (paper: table 5)
...
%Plot forward error bounds relative to dynare as scatter plot, all methods (paper: figure 4)
figure
hold on
for j=2:7
...
%Plot distribution of forward error bounds relative to dynare (paper: figure 6)
figure
hold on
for j=1:7
...
%Plot computation time relative to dynare against model size (paper: figure 5)
figure
hold on
for j=2:7
...
%Plot forward error bounds relative to dynare against model size (paper: figure 5)
figure
hold on
for j=2:7
...
%regression line (subset shown in figure 5)
for j=2:7
    hold on
    figure
scatter(log10(model_variable_number(diffs_select(j,:))),log10(rel_fe_2(j,diffs_select(j,:))),'filled');
